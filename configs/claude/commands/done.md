...
1. **Gate check — lanes only.** If `$RUN_DIR/progress.md` exists, this run has
   ...
   (`references/coordination-protocol.md` §3.6) reads `Verdict PASS` — match only
   the file's header block (the first few lines), never the whole file, since a
   `Verdict PASS` mention in prose or a quoted/fenced example anywhere in the body
   (e.g. a `CHANGES` gate's notes, or an illustrative code fence) must not clear
   the barrier:
